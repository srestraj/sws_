...
const NavBar: React.FC = ({ router }: NavBarProps) => {
  const [isClick, setIsClisk] = useState(true);

  return (
    <>
      <nav className="max-w-screen-2xl mx-auto">
        <div className="rounded-b-[37%] md:rounded-none transition-all ease-in-out w-full md:p-10 items-center flex flex-col md:flex-row  justify-center md:justify-between ">
          <Link href={"/"}>
          ...
          <ul className="md:flex  items-center justify-between">
            {router?.map((linksList) => (
              <Link href={linksList.path} key={linksList.id}>
                <li className="text-meroColor-dark-gray text-wrap text-sm  last:me-5 hover:text-meroColor-electric-indigo text-center">
                  {linksList.section}
                </li>
              </Link>
            ))}
